...
const GUAC_URL = `http://${process.env.GUAC_HOST || 'localhost'}:${process.env.GUAC_PORT || '8080'}/guacamole`;
...
async function createConnection(accountId) {
  const token = await getAdminToken();

  // Create an RDP connection for this player
  const connData = {
    parentIdentifier: 'ROOT',
    name: `arena-${accountId}`,
    protocol: 'rdp',
    parameters: {
      hostname: RDP_HOST,
      port: '3389',
...
async function getOrCreateSession(accountId) {
  ...
  // Create new session
  const { connectionId, guacToken } = await createConnection(accountId);
...
function buildWsUrl(connectionId, token) {
  ...
  // guacamole-common-js WebSocket tunnel URL
  return `ws://${guacHost}:${guacPort}/guacamole/websocket-tunnel?token=${token}&GUAC_DATA_SOURCE=mysql&GUAC_ID=${connectionId}&GUAC_TYPE=c&GUAC_WIDTH=1280&GUAC_HEIGHT=720&GUAC_DPI=96`;
}
