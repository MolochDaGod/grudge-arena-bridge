...
// Generate a unique MaNGOS username from a grudge ID
function generateUsername(grudgeId) {
  return 'GA_' + grudgeId.substring(0, 12).replace(/[^a-zA-Z0-9]/g, '');
}

// Generate a random password for the MaNGOS account
...
async function loginOrCreate(grudgeId) {
  ...
  const [result] = await db.realmd().query(
    'UPDATE account SET email = ? WHERE username = ?',
    [`grudge:${grudgeId}`, username.toUpperCase()]
  );
